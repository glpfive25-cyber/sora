...
1. 点击页面右上角的"设置"按钮
2. 在弹出的设置面板中配置：
   - **API 密钥**：你的 API 密钥（留空使用内置免费密钥）
   - **API 基础 URL**：API 服务器地址
3. 点击"保存设置"
4. 配置会保存在浏览器本地，仅你可见
...
**注意：** 即使不配置 .env 文件，应用仍可使用内置免费 API 正常运行。
...
**注意：** 部署时记得设置环境变量 `SORA_API_KEY` 和 `SORA_BASE_URL`（可选，留空使用内置免费 API）
...
2. **配置项**：
   - API 密钥（可选，留空使用内置免费密钥）
   - API 基础 URL（可选，留空使用默认地址）
